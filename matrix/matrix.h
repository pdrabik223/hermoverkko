...
static Matrix<T> Sub(const Matrix<T> &matrix_a, const Matrix<T> &matrix_b) {
  ...
  if (matrix_a.GetWidth() != matrix_b.GetWidth() || matrix_a.GetHeight() != matrix_b.GetHeight())
    throw "incorrect matrix shape";

  Matrix<T> addition(matrix_a);
  for (int i = 0; i < matrix_a.GetData().size(); i++)
    addition.GetData()[i] -= matrix_b.GetData()[i];
  return addition;
}
